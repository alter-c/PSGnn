...
from pddl.logic.base import Formula, And, Or, Not, BinaryOp, UnaryOp
...
class AStarPlanner:
    ...
    def ground_action(self):
        """生成所有可能的动作"""
        actiondas = ActionGrounder(self.domain, self.problem)
        return actiondas.ground_all_actions()
    ...
    def _apply_effect(self, state: Set[Predicate], effect: Formula) -> None:
        ...
        if isinstance(effect, Predicate):
            # 先将该谓词的否定从状态中去除
            if Not(effect) in state:
                state.remove(Not(effect))
            # 添加谓词影响
            state.add(effect)
    ...
    def plan(self) -> Tuple[List[Action], int]:
        ...
        while open_set:
            ...
            state_hash = self.state_hash(state) # 状态哈希值用于比较最小代价, set不可哈希
